...
contract GuaranteeDecoderRouterTest is Core4MicaTestBase {
    ...
    bytes32 internal constant TEST_PRIVATE_KEY_V3 =
        bytes32(
            0xA7FD35CF3AC80A878577D5EF07A420FC7D8A53CE6A22B483D659E5EFD1A2C7D1
        );
    ...
    function test_setVersionModule_andDecode_success() public {
        ...
        MockGuaranteeModuleV3.GuaranteeV3 memory g3 = _g3(
            bytes32(uint256(1)),
            ROUTED_VERSION
        );
    ...
    function test_setVersionModule_revertInvalidVersion() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                GuaranteeDecoderRouter.InvalidVersion.selector,
                uint64(0)
            )
        );
    ...
    function test_setVersionModule_revertInvalidModule() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                GuaranteeDecoderRouter.InvalidModule.selector,
                address(0)
            )
        );
    ...
    function test_decode_revertUnknownVersion() public {
        bytes memory routedPayload = abi.encode(
            ROUTED_VERSION,
            bytes("does-not-matter")
        );
        vm.expectRevert(
            abi.encodeWithSelector(
                GuaranteeDecoderRouter.UnknownVersion.selector,
                ROUTED_VERSION
            )
        );
    ...
    function test_freezeVersion_locksConfiguration() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                GuaranteeDecoderRouter.FrozenVersion.selector,
                ROUTED_VERSION
            )
        );
    ...
    function test_decode_revertOnModuleVersionMismatch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                GuaranteeDecoderRouter.ModuleVersionMismatch.selector,
                ROUTED_VERSION,
                uint64(99)
            )
        );
    ...
    function test_core4Mica_verifyAndDecodeGuarantee_withRouterVersion() public {
        ...
        bytes32 domainV3 = keccak256(
            abi.encode("4MICA_CORE_GUARANTEE_V3", block.chainid, address(core4Mica))
        );
        ...
        bytes memory outerGuarantee = abi.encode(
            ROUTED_VERSION,
            abi.encode(ROUTED_VERSION, abi.encode(g3))
        );
        BLS.G2Point memory signature = BlsHelper.blsSign(
            outerGuarantee,
            TEST_PRIVATE_KEY_V3
        );
    ...
    function _g3(
        bytes32 domain,
        uint64 version
    ) internal view returns (MockGuaranteeModuleV3.GuaranteeV3 memory) {
        return
            MockGuaranteeModuleV3.GuaranteeV3({
                domain: domain,
                tab_id: 77,
                req_id: 11,
                client: USER1,
