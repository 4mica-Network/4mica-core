...
contract Core4MicaGovernanceHardeningTest is Test {
    ...
    function _baseConfig()
        internal
        view
        returns (Core4MicaFullStackScript.DeploymentConfig memory config)
    {
    ...
    function test_finalizeHandsOffAdminWithDelayAndRenouncesDeployer() public {
        ...
        (bool deployerIsAdmin, ) = manager.hasRole(
            ADMIN_ROLE,
            address(harness)
        );
    ...
    function test_finalizeInstallsRoleGrantDelaysAfterSetback() public {
        ...
        assertEq(
            manager.getRoleGrantDelay(GOVERNANCE_ROLE),
            0,
            "grant delay active too early"
        );
        vm.warp(block.timestamp + manager.minSetback() + 1);
        assertEq(
            manager.getRoleGrantDelay(GOVERNANCE_ROLE),
            72 hours,
            "grant delay not installed"
        );
        // The collateral-seizing role a compromised admin would self-grant (4MCA-H02) is delayed too.
        assertEq(
            manager.getRoleGrantDelay(CLEARING_HOUSE_ROLE),
            72 hours,
            "clearing-house grant delay not installed"
        );
    }
    ...
    function test_validateRejectsDeployerAsAdmin() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Core4MicaFullStackScript.InsecureGovernanceAdmin.selector,
                address(harness)
            )
        );
    ...
    function test_holderCheckRejectsDeployer() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                Core4MicaFullStackScript.RoleHolderNotSeparated.selector,
                "GOVERNANCE_ROLE_HOLDER"
            )
        );
        harness.checkHolder(
            "GOVERNANCE_ROLE_HOLDER",
