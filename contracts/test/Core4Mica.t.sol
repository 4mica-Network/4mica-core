...
contract Core4MicaTest is Test {
    ...
    bytes32 public TEST_PRIVATE_KEY =
        bytes32(
            0x4573DBD225C8E065FC30FF774C9EF81BD29D34E559D80E2276EE7824812399D3
        );
    BLS.G1Point public TEST_PUBLIC_KEY =
        BlsHelper.getPublicKey(TEST_PRIVATE_KEY);
    ...
    function test_SetVerificationKey() public {
        ...
        (bytes32 x_a, bytes32 x_b, bytes32 y_a, bytes32 y_b) = core4Mica
            .GUARANTEE_VERIFICATION_KEY();
    ...
    function test_FinalizeWithdrawal_NotFullAmount() public {
        ...
        (collateral, withdrawalTimestamp, withdrawalAmount) = core4Mica.getUser(
            user1
        );
        assertEq(collateral, 1 ether);
        assertEq(withdrawalTimestamp, 0);
    ...
    function test_FinalizeWithdrawal_FullCollateral() public {
        ...
        assertEq(
            ...
            5 ether,
            "User did not receive full collateral"
        );
    }
        ...
        public
    {
        ...
        (uint256 collateral, , uint256 withdrawal_amount) = core4Mica.getUser(
            user1
        );
        assertEq(collateral, 0.5 ether);
    ...
    function test_DoubleSpend_IllegalGuarantee() public {
        ...
        assertGt(
            g.tab_timestamp,
            withdrawal_timestamp + core4Mica.synchronizationDelay()
        );
    ...
    function test_VerifyGuaranteeSignature_InvalidGuarantee() public {
        ...
        Core4Mica.Guarantee memory g2 = Core4Mica.Guarantee(
            0x1234,
            vm.getBlockTimestamp(),
            user1,
            user2,
    ...
    function test_VerifyGuaranteeSignature_InvalidSigningKey() public {
        ...
        assert(
            !core4Mica.verifyGuaranteeSignature(g, signature_with_other_key)
        );
    }
    ...
    function test_Receive_Reverts_TransferFailed() public {
        vm.prank(user1);
        (bool ok, bytes memory mem) = address(core4Mica).call{
            value: 0.25 ether
        }("");
        assert(!ok);
        assertEq(
