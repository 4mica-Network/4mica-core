...
import {
    FourMicaServiceManager,
    IServiceManager,
    IFourMicaTaskManager
} from "../../src/FourMicaServiceManager.sol";
import {FourMicaTaskManager} from "../../src/FourMicaTaskManager.sol";
...
library FourMicaDeploymentLib {
    ...
    struct DeploymentData {
        address fourMicaServiceManager;
        address fourMicaTaskManager;
    ...
    function deployContracts(
        ...
        address strategy,
        FourMicaSetupConfig memory isConfig,
        address admin
        ...
        result.fourMicaServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.fourMicaTaskManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        ...
        address registryCoordinatorImpl = address(
            new RegistryCoordinator(
                IServiceManager(result.fourMicaServiceManager),
        ...
        FourMicaServiceManager fourMicaServiceManagerImpl = new FourMicaServiceManager(
            (IAVSDirectory(avsdirectory)),
            IRegistryCoordinator(result.registryCoordinator),
            IStakeRegistry(result.stakeRegistry),
            core.rewardsCoordinator,
            IFourMicaTaskManager(result.fourMicaTaskManager)
        );
        UpgradeableProxyLib.upgrade(
            result.fourMicaServiceManager, address(fourMicaServiceManagerImpl)
        );
        FourMicaServiceManager(result.fourMicaServiceManager).initialize(admin, admin);
        bytes memory taskmanagerupgradecall = abi.encodeCall(
            FourMicaTaskManager.initialize, (IPauserRegistry(address(pausercontract)), admin, admin)
        );
        UpgradeableProxyLib.upgradeAndCall(
            result.fourMicaTaskManager,
            address(new FourMicaTaskManager(IRegistryCoordinator(result.registryCoordinator), 30)),
    ...
    function readDeploymentJson(uint256 chainId) internal returns (DeploymentData memory) {
        return readDeploymentJson("script/deployments/fourmica/", chainId);
    }

    function readFourMicaConfigJson(string memory directoryPath)
        internal
        returns (FourMicaSetupConfig memory)
    {
        string memory fileName = string.concat(directoryPath, ".json");
        require(vm.exists(fileName), "Deployment file does not exist");
        string memory json = vm.readFile(fileName);

        FourMicaSetupConfig memory data;
        ...
        returns (DeploymentData memory)
    {
        ...
        data.fourMicaServiceManager = json.readAddress(".addresses.FourMicaServiceManager");
        data.fourMicaTaskManager = json.readAddress(".addresses.FourMicaTaskManager");
    ...
    function writeDeploymentJson(DeploymentData memory data) internal {
        writeDeploymentJson("script/deployments/fourmica/", block.chainid, data);
    }

    function writeDeploymentJson(string memory outputPath, uint256 chainId, DeploymentData memory data) internal {
        address proxyAdmin = address(UpgradeableProxyLib.getProxyAdmin(data.fourMicaServiceManager));
        ...
        returns (string memory)
    {
        return string.concat(
            ...
            '","FourMicaServiceManager":"',
            data.fourMicaServiceManager.toHexString(),
            '","fourMicaServiceManagerImpl":"',
            data.fourMicaServiceManager.getImplementation().toHexString(),
            '","FourMicaTaskManager":"',
            data.fourMicaTaskManager.toHexString(),
            '","registryCoordinator":"',
