...
contract WriteToContractsRegistry {
    ...
    FourMicaDeploymentLib.DeploymentData internal avsConfigData;
    ...
    function run() public virtual {
        ...
        avsConfigData =
            FourMicaDeploymentLib.readDeploymentJson("script/deployments/incredible-squaring/", block.chainid);
        vm.startBroadcast();
        if (block.chainid == 31337 || block.chainid == 1337) {
            _writeCoreContractsToRegistry(coreConfigData);
            _writeFourMicaContractsToRegistry(avsConfigData);
        }
    ...
    function _writeFourMicaContractsToRegistry(
        FourMicaDeploymentLib.DeploymentData memory deploymentdata
    ) internal {
        contractsRegistry.registerContract(
            "fourmica_task_manager", address(deploymentdata.fourMicaTaskManager)
        );
        contractsRegistry.registerContract("erc20MockStrategy", address(deploymentdata.strategy));
        contractsRegistry.registerContract(
            "fourmica_registry_coordinator", address(deploymentdata.registryCoordinator)
        );
        contractsRegistry.registerContract(
            "fourmica_operator_state_retriever", address(deploymentdata.operatorStateRetriever)
        );
    }
}
