...
# Stablecoin + validation-registry inputs for the constructor. These only need
# to be distinct, non-zero addresses for the local stack (no Aave wiring).
STABLECOIN_0="${STABLECOIN_0:-0x1111111111111111111111111111111111111111}"
STABLECOIN_1="${STABLECOIN_1:-0x2222222222222222222222222222222222222222}"
...
deploy_contracts() {
  ...
  (
    ...
    STABLECOINS_COUNT=2 STABLECOIN_0="$STABLECOIN_0" STABLECOIN_1="$STABLECOIN_1" \
...
HEALTH_CHECK_CRON="*/5 * * * * *"
EOF
  ok "wrote $ENV_FILE (contract $contract_addr)"
}
