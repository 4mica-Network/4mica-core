...
sol! {
    ...
    contract Core4Mica {
        ...
        /// The GuaranteeVerifier paired with this vault at deployment (immutable).
        ...
        /// @param guaranteeVerifier_ Address of the paired GuaranteeVerifier
        /// @param stablecoins_ Initially supported stablecoin assets
        /// @param minGracePeriod_ On-chain floor for the withdrawal grace period
        constructor(
            address manager,
            address guaranteeVerifier_,
...
sol! {
    ...
    /// deployed bytecode stays clear of the BLS pairing code. Resolve its address from
    /// `Core4Mica.guaranteeVerifier()`.
    #[sol(rpc)]
    contract GuaranteeVerifier {
        ...
        function INITIAL_GUARANTEE_VERSION() external view returns (uint64);
        function getGuaranteeVersionConfig(uint64 version) external view returns (
