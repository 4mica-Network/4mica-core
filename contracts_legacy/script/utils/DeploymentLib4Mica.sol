...
library DeploymentLib4Mica {
    ...
    function deployContracts(
        ...
        address strategy,
        SetupConfig4Mica memory isConfig,
        address admin
        ...
        result.serviceManager4Mica = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        ...
        address registryCoordinatorImpl = address(
            new RegistryCoordinator(
                IServiceManager(result.serviceManager4Mica),
        ...
        ServiceManager4Mica serviceManager4MicaImpl = new ServiceManager4Mica(
            (IAVSDirectory(avsdirectory)),
            IRegistryCoordinator(result.registryCoordinator),
            IStakeRegistry(result.stakeRegistry),
            core.rewardsCoordinator,
            IServiceManager4Mica(result.serviceManager4Mica)
        );
        UpgradeableProxyLib.upgrade(result.serviceManager4Mica, address(serviceManager4MicaImpl));
        ServiceManager4Mica(result.serviceManager4Mica).initialize(admin, admin);
        bytes memory taskmanagerupgradecall =
            abi.encodeCall(ServiceManager4Mica.initialize, (IPauserRegistry(address(pausercontract)), admin, admin));
        UpgradeableProxyLib.upgradeAndCall(
            result.serviceManager4Mica,
            address(new ServiceManager4Mica(IRegistryCoordinator(result.registryCoordinator), 30)),
    ...
    function readDeploymentJson(uint256 chainId) internal returns (DeploymentData memory) {
        return readDeploymentJson("script/deployments/4mica/", chainId);
    }

    function read4MicaConfigJson(string memory directoryPath) internal returns (SetupConfig4Mica memory) {
        string memory fileName = string.concat(directoryPath, ".json");
        require(vm.exists(fileName), "Deployment file does not exist");
        string memory json = vm.readFile(fileName);

        SetupConfig4Mica memory data;
        ...
        returns (DeploymentData memory)
    {
        ...
        data.serviceManager4Mica = json.readAddress(".addresses.ServiceManager4Mica");
        data.serviceManager4Mica = json.readAddress(".addresses.ServiceManager4Mica");
    ...
    function writeDeploymentJson(DeploymentData memory data) internal {
        writeDeploymentJson("script/deployments/4mica/", block.chainid, data);
    }

    function writeDeploymentJson(string memory outputPath, uint256 chainId, DeploymentData memory data) internal {
        address proxyAdmin = address(UpgradeableProxyLib.getProxyAdmin(data.serviceManager4Mica));
        ...
        returns (string memory)
    {
        return string.concat(
            ...
            '","ServiceManager4Mica":"',
            data.serviceManager4Mica.toHexString(),
            '","serviceManager4MicaImpl":"',
            data.serviceManager4Mica.getImplementation().toHexString(),
            '","ServiceManager4Mica":"',
            data.serviceManager4Mica.toHexString(),
            '","registryCoordinator":"',
