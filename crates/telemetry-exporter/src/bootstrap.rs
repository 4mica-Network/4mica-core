...
pub async fn run() -> anyhow::Result<()> {
    ...
    info!(
        "telemetry exporter config: bind_addr={}, chain_id={}, snapshot_interval_sec={}, query_timeout_ms={}, max_db_connections={}, stale_after_sec={}, probe_query_sql={}",
        cfg.bind_addr(),
        cfg.chain_id,
    ...
    telemetry::init_metric_namespace(&cfg.chain_id)?;
    telemetry::emit_startup_metrics();

    let readonly_db =
        db::connect_readonly_pool(&cfg.readonly_replica_dsn, cfg.max_db_connections).await?;
    db::ensure_readonly_session(&readonly_db, cfg.query_timeout_ms).await?;
    info!(
    ...
    let probe_query_sql = cfg.probe_query_sql.clone();
